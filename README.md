...
- **ライブチャンネルの優先表示**: タイムライン表示の際、現在ライブ配信中のチャンネルがリストの一番上に表示されます。
...
- **アフィリエイトリンク**: タイムラインの上に、Amazonで販売されているゲーミング関連商品をランダムで紹介するアフィリエイトリンクが表示されます。
...
- **`script.js`の編集**:
  - **TwitchクライアントID**: 3行目にある `const CLIENT_ID = '...';` の値を、手順1で取得した自身の**Client ID**に書き換えます。
  - **Amazonアソシエイトタグ**: `setupAffiliateLink`関数内にある `const affiliateTag = '...';` の値を、あなた自身の**AmazonアソシエイトのトラッキングID**に書き換えます。
- **リダイレクトURLの設定**:
